...
fun ICorChainDsl<InternalContext>.repoReadCompositionByText(title: String) = worker {
    this.title = title
    description = "Чтение состава из БД"
    on { state == IsState.RUNNING}
    handle {
        try {
            compositionResponse = compositionRepo?.findByText(compositionTextRequest)
                ?: throw RuntimeException("Composition not found. Text:$compositionTextRequest")
        } catch (e: Throwable) {
            fail(
                errorRepo(
                    field = "composition",
